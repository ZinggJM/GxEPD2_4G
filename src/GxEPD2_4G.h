...
#if ENABLE_GxEPD2_GFX
class GxEPD2_4G_R : public GxEPD2_GFX
#else
class GxEPD2_4G_R : public Adafruit_GFX
#endif
{
  public:
    GxEPD2_Type& epd2;
#if ENABLE_GxEPD2_GFX
    GxEPD2_4G_R(GxEPD2_Type& epd2_instance) : GxEPD2_GFX(epd2, GxEPD2_Type::WIDTH, GxEPD2_Type::HEIGHT), epd2(epd2_instance)
#else
    GxEPD2_4G_R(GxEPD2_Type & epd2_instance) : Adafruit_GFX(GxEPD2_Type::WIDTH, GxEPD2_Type::HEIGHT), epd2(epd2_instance)
#endif
    {
        ...
        case 2:
          {
            ...
            for (int16_t j = 0; j < h; j++)
            {
              for (int16_t i = 0; i < w; i++ )
              {
                ...
                // rrrr'rggg'gggb'bbbb                    rrrrrggggggbbbbb
                uint16_t color = ((byte & 0xC0) >> 4) * 0b0100001000001000;
                drawPixel(x + i, y + j, color);
              }
            }
          }
        ...
        case 4:
          {
            ...
            for (int16_t j = 0; j < h; j++)
            {
              for (int16_t i = 0; i < w; i++ )
              {
                ...
                // rrrr'rggg'gggb'bbbb                    rrrrrggggggbbbbb
                uint16_t color = ((byte & 0xF0) >> 4) * 0b0001000010000010;
                drawPixel(x + i, y + j, color);
              }
            }
          }
...
#endif
                // rrrr'rggg'gggb'bbbb                    rrrrrggggggbbbbb
                uint16_t color = ((byte & 0xF8) >> 3) * 0b0000100001000001;
                drawPixel(x + i, y + j, color);
              }
            }
          }
...
template<typename GxEPD2_Type, const uint16_t page_height>
class GxEPD2_4G : public GxEPD2_4G_R<GxEPD2_Type, page_height>
{
  private:
    GxEPD2_Type epd2_copy;
  public:
    GxEPD2_4G(GxEPD2_Type epd2_instance) : GxEPD2_4G_R<GxEPD2_Type, page_height>(epd2_copy), epd2_copy(epd2_instance)
    {
    }
};
