...
void GxEPD2_370_TC1::clearScreenArea(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
  if (!_init_display_done && !_init_4G_done) _InitDisplay();
  uint8_t value = (_refresh_mode == grey_refresh) ? 0x00 : 0xFF;
  _writeScreenBufferArea(0x24, value, x, y, w, h);
  _writeScreenBufferArea(0x26, value, x, y, w, h);
  refresh(x, y, w, h);
}

void GxEPD2_370_TC1::_writeScreenBufferArea(uint8_t command, uint8_t value, uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
...
void GxEPD2_370_TC1::writeImage(const uint8_t bitmap[], int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y, bool pgm)
{
  ...
  {
    // this would clean the background to white
    //_Force_Init_Full();
    //_writeImage(0x26, bitmap, x, y, w, h, invert, mirror_y, pgm);
    // this allows fast partial updates on grey background
    clearScreenArea(x, y, w, h); // white window using grey mode
    _InitDisplay();
    _Init_Part();
  }
...
void GxEPD2_370_TC1::writeImage_4G(const uint8_t bitmap[], uint8_t bpp, int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y, bool pgm)
{
...
void GxEPD2_370_TC1::writeImagePart_4G(const uint8_t bitmap[], uint8_t bpp, int16_t x_part, int16_t y_part, int16_t w_bitmap, int16_t h_bitmap,
                                       int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y, bool pgm)
{
  uint16_t ppb = (bpp == 2 ? 4 : (bpp == 4 ? 2 : (bpp == 8 ? 1 : 0)));
  uint8_t mask = (bpp == 2 ? 0xC0 : (bpp == 4 ? 0xF0 : 0xFF));
...
void GxEPD2_370_TC1::drawImage_4G(const uint8_t bitmap[], uint8_t bpp, int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y, bool pgm)
{
...
void GxEPD2_370_TC1::drawImagePart_4G(const uint8_t bitmap[], uint8_t bpp, int16_t x_part, int16_t y_part, int16_t w_bitmap, int16_t h_bitmap,
                                      int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y, bool pgm)
{
...
void GxEPD2_370_TC1::refresh(bool partial_update_mode)
{
  ...
  else
  {
    ...
    {
      if (_refresh_mode != full_refresh) _Init_Full();
      _Update_Full();
    }
...
void GxEPD2_370_TC1::_InitDisplay()
{
  ...
  } else _refresh_mode = unset_refresh;
}
...
const unsigned char GxEPD2_370_TC1::lut_4G[] PROGMEM =
{
...
void GxEPD2_370_TC1::_Force_Init_Full()
{
  _InitDisplay();
  _Init_Full();
  _refresh_mode = forced_full_refresh;
...
void GxEPD2_370_TC1::_Init_4G()
{
  if (_hibernating) _reset();
  delay(10); // 10ms according to specs
  _writeCommand(0x12);
  //_waitWhileBusy("_Init_4G 1", power_on_time); // 1ms
  delay(2);
  _writeCommand(0x46);
  _writeData(0xF7);
  //_waitWhileBusy("_Init_4G 2", power_on_time); // 7ms
  delay(10);
  _writeCommand(0x47);
  _writeData(0xF7);
  //_waitWhileBusy("_Init_4G 3", power_on_time); // 7ms
  delay(10);
  _writeCommand(0x01); // setting gate number
  _writeData(0xDF);
  _writeData(0x01);
  _writeData(0x00);
  ...
  _writeData(0x88);    // BW and RED inversed
...
void GxEPD2_370_TC1::_Update_4G()
{
  _writeCommand(0x22);
  _writeData(0xc7);
  _writeCommand(0x20);
