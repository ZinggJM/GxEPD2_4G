...
class GxEPD2_370_TC1 : public GxEPD2_4G_EPD
{
  public:
    // attributes
    ...
    static const uint16_t grey_refresh_time = 2000; // ms, e.g. 1740000us
    static const uint16_t full_refresh_time = 1000; // ms, e.g. 981000us
    static const uint16_t partial_refresh_time = 800; // ms, e.g. 781000us
    // constructor
    ...
    void clearScreenArea(uint16_t x, uint16_t y, uint16_t w, uint16_t h); // to white
  ...
  private:
    ...
    void _Update_Part();
  private:
    enum {full_refresh, grey_refresh, fast_refresh, forced_full_refresh, unset_refresh} _refresh_mode;
